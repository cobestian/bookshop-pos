...
app.get("/customers/:shopId", async (req, res) => {
  try {
    const rows = await query(
      "SELECT * FROM customers WHERE shop_id = ? ORDER BY account_name ASC",
      [req.params.shopId]
    );
    res.json(rows);
  } catch (err) {
...
app.get("/sales/:shopId", async (req, res) => {
  try {
    const sales = await query(
      "SELECT * FROM sales WHERE shop_id = ? ORDER BY sale_date DESC",
      [req.params.shopId]
    );
    for (const sale of sales) {
      sale.items = await query(
        `SELECT si.*, p.name as product_name, p.supplier
         FROM sales_items si
...
app.post("/sales", async (req, res) => {
  const { shopId, enteredBy, total, items } = req.body;
  try {
    const saleResult = await query(
      "INSERT INTO sales (shop_id, entered_by, total) VALUES (?, ?, ?)",
      [shopId, enteredBy, total]
    );
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (existing.length > 0) {
        await query(
          "UPDATE products SET qty = qty + ?, cost = ?, selling = ? WHERE id = ?",
...
app.get("/customer-payments/:shopId", async (req, res) => {
  try {
    const rows = await query(
      "SELECT * FROM customer_payments WHERE shop_id = ? ORDER BY created_at DESC",
      [req.params.shopId]
    );
    res.json(rows);
  } catch (err) {
